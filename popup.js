...
document.getElementById('sendButton').addEventListener('click', createConnection);
document.getElementById('receiveButton').addEventListener('click', receiveConnection);
...
function createConnection() {
    ...
    localConnection.onicecandidate = e => {
        if (e.candidate) {
            ...
            sendToRemotePeer({ type: 'candidate', candidate: e.candidate, target: targetClientId });
        }
    };
...
function receiveConnection() {
    ...
    remoteConnection.onicecandidate = e => {
        if (e.candidate) {
            console.log('Sending ICE candidate.');
            sendToLocalPeer({ type: 'candidate', candidate: e.candidate, target: targetClientId });
        }
    };
    ...
    receiveOfferFromLocalPeer().then(offer => {
...
function sendFile(file) {
    ...
    function sendChunk(chunk) {
        try {
            ...
            if (error.name === 'OperationError' && sendChannel.bufferedAmount > sendChannel.bufferedAmountLowThreshold) {
    ...
    return new Promise(resolve => {
        signalingServer.onmessage = message => {
            ...
            if (data.type === 'offer') {
                ...
                remoteConnection.addIceCandidate(new RTCIceCandidate(data.candidate)).catch(error => {
    ...
    if (data.type === 'offer') {
        ...
        if (localConnection) {
            ...
            remoteConnection.addIceCandidate(candidate).catch(error => {
                console.error('Error adding ICE candidate to remote connection:', error);
            });
        }
    }
}